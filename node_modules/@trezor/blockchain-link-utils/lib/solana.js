...
const transformTokenInfo = (tokenAccounts, tokenDetailByMint) => {
    const tokens = ts_belt_1.F.toMutable((0, ts_belt_1.pipe)(tokenAccounts, ts_belt_1.A.filter(isSplTokenAccount), ts_belt_1.A.map(tokenAccount => {
        ...
        return Object.assign(Object.assign({ type: 'SPL', contract: info.mint, balance: info.tokenAmount.amount, decimals: info.tokenAmount.decimals }, (0, exports.getTokenNameAndSymbol)(info.mint, tokenDetailByMint)), { address: tokenAccount.pubkey.toString() });
...
const extractAccountBalanceDiff = (transaction, address, isTokenDiff = false) => {
    ...
    const pubKeyIndex = transaction.transaction.message.accountKeys.findIndex(ak => ak.pubkey.toString() === address);
    if (pubKeyIndex === -1) {
        return null;
    }
    if (isTokenDiff) {
...
function getNativeEffects(transaction) {
    ...
    return transaction.transaction.message.accountKeys
        ...
        if (wSolTransferInstruction && 'parsed' in wSolTransferInstruction) {
            if (wSolTransferInstruction.parsed.info.destination === targetAddress ||
...
const getNativeTransferTxType = (effects, accountAddress, transaction) => {
    ...
    if (effects.length === 1 &&
        ...
        ((_b = effects[0]) === null || _b === void 0 ? void 0 : _b.amount.abs().isEqualTo(new bigNumber_1.BigNumber(((_c = transaction.meta) === null || _c === void 0 ? void 0 : _c.fee) || 0)))) {
        return 'self';
    }
...
const getDetails = (transaction, effects, accountAddress, txType) => {
    ...
    return {
        size: ((_a = transaction.meta) === null || _a === void 0 ? void 0 : _a.computeUnitsConsumed) || 0,
        totalInput: senders
...
const getTokens = (tx, accountAddress, tokenDetailByMint, tokenAccountsInfos) => {
    ...
    const effects = tx.transaction.message.instructions
        ...
        const decimals = ((_a = parsed.info.tokenAmount) === null || _a === void 0 ? void 0 : _a.decimals) || (instructionTokenInfo === null || instructionTokenInfo === void 0 ? void 0 : instructionTokenInfo.decimals) || 0;
